...
Rectangle {
  ...
  property string itemPath: "" // <--- Bắt buộc phải là itemPath để khớp với FileBrowserCard.qml
  ...
  MouseArea {
    ...
    onClicked: {
      console.log("[DEBUG] Đã click:", root.label, " | Path nhận được:", root.itemPath);

      if (!root.itemPath || root.itemPath === "") {
        console.log("[DEBUG] LỖI: itemPath bị rỗng!");
        return;
      }
